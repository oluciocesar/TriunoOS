...
Você acaba de instalar o TriunoOS. Em alguns minutos, sua empresa vai
...
```
Clona o https://github.com/oluciocesar/TriunoOS.git na pasta atual,
...
```
git clone https://github.com/oluciocesar/TriunoOS.git
cd TriunoOS
code .
```
...
Quando o `/instalar` terminar, renomeia a pasta `TriunoOS/` pro nome do teu
negócio (fecha o VS Code, renomeia no Explorer/Finder, abre de novo). A
pasta não fica como "TriunoOS" — ela é o teu negócio agora.
...
closed loop dentro do TriunoOS (decide → executa → captura → realimenta →
ajusta sozinho).
